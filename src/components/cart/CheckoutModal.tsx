...
const CheckoutModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Reset State on Open to prevent stale "Processing" state
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const order = await createOrder({
        items: items,
        total: totals.total,
        note: "", // We could add a note field later
      });
                  ...
                  <div key={`${item.id}-${idx}`} className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        <h4 className="font-bold text-lg leading-tight text-gray-900 dark:text-white">
                          {item.name}
                        </h4>
