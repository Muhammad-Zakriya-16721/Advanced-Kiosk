...
export const useKeyboardControls = (
  ...
  }, [filteredItems, cartItems, activeSection]);
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    if (activeSection === SECTIONS.CART) {
       ...
       if (e.key === 'Backspace') {
           e.preventDefault();
           // "remove top selected item" -> Remove focused item (reduce qty by 1? or remove? Prompt says "remove top selected item", 
           // but Sidebar says "Backspace will remove the quantity by one" in Main.
           // In Cart: "Backspace will remove the top selected item". "Left key will reduce...".
           // This implies Backspace REMOVES it (Quantity -> 0).
